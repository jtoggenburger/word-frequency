...
private:
   ...
   void inOrderString(std::string& s, Node* R, Node* NIL) const;

   // preOrderString()
   // Appends a string representation of the tree rooted at R to s. The appended
   // string consists of keys only, separated by "\n", with the order determined
   // by a pre-order tree walk.
   void preOrderString(std::string& s, Node* R, Node* NIL) const;
   ...
   void postOrderDelete(Node* R, Node* NIL);

   // search()
   // Searches the subtree rooted at R for a Node with key==k. Returns
   // the address of the Node if it exists, returns nil otherwise.
   Node* search(Node* R, keyType k, Node* NIL) const;

   // findMin()
   // If the subtree rooted at R is not empty, returns a pointer to the 
   // leftmost Node in that subtree, otherwise returns nil.
   Node* findMin(Node* R, Node* NIL);

   // findMax()
   // If the subtree rooted at R is not empty, returns a pointer to the 
   // rightmost Node in that subtree, otherwise returns nil.
   Node* findMax(Node* R, Node* NIL);

   // findNext()
   // If N does not point to the rightmost Node, returns a pointer to the
   // Node after N in an in-order tree walk.  If N points to the rightmost 
   // Node, or is nil, returns nil. 
   Node* findNext(Node* N, Node* NIL);

   // findPrev()
   // If N does not point to the leftmost Node, returns a pointer to the
   // Node before N in an in-order tree walk.  If N points to the leftmost 
   // Node, or is nil, returns nil.
   Node* findPrev(Node* N, Node* NIL);

   // RBT Helper Functions (Optional) -----------------------------------------
   
   // LeftRotate()
   void LeftRotate(Node* N, Node* NIL);

   // RightRotate()
   void RightRotate(Node* N, Node* NIL);

   // RB_InsertFixUP()
   void RB_InsertFixUp(Node* N, Node* NIL);

   // RB_Transplant()
   void RB_Transplant(Node* u, Node* v, Node* NIL);

   // RB_DeleteFixUp()
   void RB_DeleteFixUp(Node* N, Node* NIL);

   // RB_Delete()
   void RB_Delete(Node* N, Node* NIL);
